...
export const match = (opts, callback) => {
	...
	if ((name && artist_name) || (name && album_name) || name) {
		...
		const url_match = SPOTIFY_SEARCH + common + SPOTIFY_LAST;
...
export const radio = (opts, callback) => {
	const radio = opts.radio || Type.SPOTIFY_RADIO_TYPE.WORLD;
	const url_radio = SPOTIFY_BASE + 'radio/' + radio + SPOTIFY_LAST;
